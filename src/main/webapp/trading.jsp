    ...
    <table class="w-full text-right border-collapse market-table">
        ...
        <c:forEach var="s" items="${marketList}">
            ...
            <tr onclick="openStockModal('${s.symbol}', ${s.ceilingPrice}, ${s.floorPrice}, ${s.refPrice}, ${s.currentPrice}, ${s.changePercent}, ${s.volume != null ? s.volume : 0})">
                ...
                <td class="text-gray-400 text-center"><c:if test="${s.topBuy[2].totalVolume > 0}"><fmt:formatNumber value="${s.topBuy[2].totalVolume * 10}"/></c:if></td>
                ...
                <td class="text-gray-400 text-center"><c:if test="${s.topBuy[1].totalVolume > 0}"><fmt:formatNumber value="${s.topBuy[1].totalVolume * 10}"/></c:if></td>
                ...
                <td class="text-white font-bold border-r border-dark-700 text-center"><c:if test="${s.topBuy[0].totalVolume > 0}"><fmt:formatNumber value="${s.topBuy[0].totalVolume * 10}"/></c:if></td>
                ...
                <td class="text-white font-bold text-center border-r border-dark-700"><c:if test="${s.topSell[0].totalVolume > 0}"><fmt:formatNumber value="${s.topSell[0].totalVolume * 10}"/></c:if></td>
                ...
                <td class="text-gray-400 text-center border-r border-dark-700"><c:if test="${s.topSell[1].totalVolume > 0}"><fmt:formatNumber value="${s.topSell[1].totalVolume * 10}"/></c:if></td>
                ...
                <td class="text-gray-400 text-center"><c:if test="${s.topSell[2].totalVolume > 0}"><fmt:formatNumber value="${s.topSell[2].totalVolume * 10}"/></c:if></td>
    ...
    <div class="bg-dark-900 w-full max-w-[1400px] h-[90vh] rounded-lg border border-dark-700 flex flex-col modal-content overflow-hidden animate-fade-in-up">
        ...
        <div class="flex-1 grid grid-cols-12 overflow-hidden bg-dark-900">
            ...
            <div class="col-span-12 lg:col-span-2 flex flex-col border-r border-dark-700 bg-dark-800/30">
                ...
                <div class="flex-1 overflow-y-auto font-mono text-[11px]">
                    ...
                    <div class="space-y-0.5 pt-1">
                        ...
                        <div class="flex justify-between items-center px-2 py-1 relative group hover:bg-dark-700/50 cursor-pointer"><span class="w-1/3"></span><span class="w-1/3 text-center color-ref font-bold">26.20</span><span class="w-1/3 text-right text-white font-bold">15,600</span><div class="absolute right-0 top-0 bottom-0 bg-red-500/25 w-[45%]"></div></div>
                    </div>
                    ...
                    <div class="space-y-0.5 pb-1">
                        ...
                        <div class="flex justify-between items-center px-2 py-1 relative group hover:bg-dark-700/50 cursor-pointer"><span class="w-1/3 text-left text-gray-300">45,200</span><span class="w-1/3 text-center color-up font-bold">26.05</span><span class="w-1/3"></span><div class="absolute left-0 top-0 bottom-0 bg-green-500/15 w-[50%]"></div></div>
                        <div class="flex justify-between items-center px-2 py-1 relative group hover:bg-dark-700/50 cursor-pointer"><span class="w-1/3 text-left text-gray-300">12,500</span><span class="w-1/3 text-center color-ref font-bold">26.00</span><span class="w-1/3"></span><div class="absolute left-0 top-0 bottom-0 bg-green-500/15 w-[20%]"></div></div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-span-12 lg:col-span-3 bg-dark-800 flex flex-col relative">
                ...
                <form id="orderForm" action="place-order" method="POST" class="px-4 flex flex-col gap-4">
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        <button type="button" id="btnLO" onclick="selectOrderType('LO')" class="border text-xs font-bold py-2 rounded transition btn-selected">LO</button>
                        <button type="button" id="btnMP" onclick="selectOrderType('MP')" class="border text-xs font-bold py-2 rounded transition btn-unselected">MP</button>
                    </div>
...
<script>
    ...
    // --- GLOBAL VARS (Đã dùng ID cụ thể cho nút Submit) ---
    const tabBuy = document.getElementById('tabBuy');
    const tabSell = document.getElementById('tabSell');
    const btnSubmit = document.getElementById('btnSubmitOrder'); // [FIXED] Chọn đúng nút submit
    ...
    // --- LOGIC CHỌN LOẠI LỆNH (LO/MP) ---
    function selectOrderType(type) {
        hiddenOrderType.value = type;

        if (type === 'LO') {
            // Đổi class để hiển thị nút Active
    ...
    // --- CHART & MODAL (Giữ nguyên) ---
    function initChart() { /* Code Chart cũ */
